			...
			<c:choose>
				<c:when test="${param.nickname == authUser.nickname}">
					<ul class="nav nav-tabs">
						...
						<li role="presentation" class="active"><a href="${pageContext.request.contextPath}/${authUser.nickname}/like_playlist">플레이리스트</a></li>
			...
			<c:forEach items="${requestScope.foldermainMap.playList}" var="playlistVo">
				...
				<div class="jumbotron">
					<div id="reviewVo-wrap">
						<div id="review_first">
							<h3>
								<a href="${pageContext.request.contextPath}/bookdetail?bookNo=${playlistVo.bookNo}&userNo=${playlistVo.userNo}">${playlistVo.bookTitle}</a>
							</h3>

							<!-- 자기글에만 수정 삭제 노출 -->
							<c:if test="${authUser.userNo == playlistVo.userNo}">
								<a id="${playlistVo.reviewNo }" class="review_modify btn_delete_review" data-reviewno="${playlistVo.reviewNo}">삭제</a>
						...
						<div id="review_second">
							...
							<span class="review_like">${playlistVo.likecnt}</span>
							<span class="review_like">${playlistVo.reviewDate}</span> 
			...
			<nav id="page">
				<ul class="pagination">
					...
					<c:if test="${foldermainMap.prev == true}">
						<li>
							<a href="${pageContext.request.contextPath}/playlist/folder?playlistNo=${foldermainMap.playlistCover.playlistNo}&userNo=${foldermainMap.playlistCover.userNo}&nickname=${param.nickname}&crtPage=${foldermainMap.startPageBtnNo-1}"
								aria-label="Previous"> 
					...
					<c:if test="${foldermainMap.next == true}">
						<li>
							<a href="${pageContext.request.contextPath}/playlist/folder?playlistNo=${foldermainMap.playlistCover.playlistNo}&userNo=${foldermainMap.playlistCover.userNo}&nickname=${param.nickname}&crtPage=${foldermainMap.endPageBtnNo+1}"
								aria-label="Next"> 
...
<script src="${pageContext.request.contextPath}/asset/js/more.js" defer></script>
<script src="${pageContext.request.contextPath}/asset/js/playlistFolder.js" defer></script>
</html>